...
class AirCraftRoutingRandom(discrete.DiscreteEnv):
    ...
    def __init__(self, desc=None):
        ...
        fs = '{0:0' + str(K) + 'b}'
        def stormID2BoolList(id): return [int(x) > 0 for x in fs.format(id)]
        # calc distribution probability, treat p to be the probability of flip, 1-p fr hold
        def calcP(n, r, p): return (p ** r) * ((1 - p) ** (n - r))
        # (row, col, storm_index) to stateID
        def to_s(row, col, storm_index): return nrow * ncol * storm_index + row * ncol + col
        # step
        def inc(row, col, a):
            if a == 0:  # left
                col = max(col - 1, 0)
        ...
        for i in range(tk):
            ...
            for j in range(tk):
                ...
                flip_bool_list = [bool_list_i[z] ^ bool_list_j[z] for z in range(k)]
                    ...
                    for new_storm_id in range(tk):
                        for a in range(4):
                            ...
                            P[old_state_id][a].append((
                                Ps[old_storm_id, new_storm_id],
                                new_state_id,
                                cost,
                                done1 or done2
                                ))
                            Pmatrix[old_state_id, a, new_state_id] = Ps[old_storm_id][new_storm_id]
                            Q[old_state_id][a].append((
                                Qs[old_storm_id, new_storm_id],
                                new_state_id,
                                cost,
                                done1 or done2
                                ))
                            Qmatrix[old_state_id, a, new_state_id] = Qs[old_storm_id][new_storm_id]
